...
def handle_message(msg):
    ...
    match msg.get("type"):
        ...
        case "GAME_OVER":
            winner = msg["winner"]
            # If winner is me, print "You", else print their ID
            print(
                f"[GAME OVER] {'You' if winner == my_id else winner} {'win' if winner == my_id else 'wins'}!"
            )
...
def draw_lobby():
    ...
    text1 = font_small.render(
        "READY" if not ready else "WAITING...", True, (0, 255, 0)
    )  # green text
...
def draw_in_game():
    # Health packs: draw as cyan squares
    for x, y in health_packs.values():
        pygame.draw.rect(window, (0, 255, 255), (x - 5, y - 5, 10, 10))
    ...
    for b in bullets:
        ...
        pygame.draw.circle(
            window, (255, 255, 0), (int(b["x"]), int(b["y"])), 4
        )  # yellow bullet
    ...
    for pid, player in players.items():
        ...
        pygame.draw.rect(
            window, (100, 100, 100), (bx, by, bar_w, bar_h)
        )  # grey background
